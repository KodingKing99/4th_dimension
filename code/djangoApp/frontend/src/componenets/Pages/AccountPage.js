...
const AccountPage = (props) => {
    ...
    const [userAccount, setUserAccount] = useState(user.account);
    const [userFirst, setUserFirst] = useState(user.firstName);
    const [userLast, setUserLast] = useState(user.lastName);
    return ( 
        <div className="account-page" style={{marginTop: '100px', textAlign: 'center'}}>
            <h1> Account Details </h1>
            <h2> Name: {userFirst} {userLast}</h2>
            <h2> Email: {user.email} </h2>
            <h2> Account Balance: ${userAccount}</h2>
    ...
    function DepositModal() {
        ...
        const handleSubmit = () => {
            ...
            setOpen(false);
            setUserAccount((parseFloat(userAccount) + parseFloat(amount)).toFixed(2));
        };
